...
class RuntimeEngine:
    ...
    async def run_once(self) -> RuntimeStatus:
        ...
        try:
            ...
            self.last_error = "runtime_cycle_failed"
    ...
    async def _process_book(self, book: str) -> None:
        candles = await self.market_data.get_candles(book, self.config.timeframe, 1)
        if not candles:
            return
        candle = candles[-1]
        ...
        self.last_decision = decision.to_public_dict()
        order = None
        ...
        self.last_order = order.to_public_dict() if order else None
        self.system.events.publish(
            "INFO",
            "Market",
            "runtime",
